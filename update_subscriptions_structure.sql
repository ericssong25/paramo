-- =====================================================
-- ACTUALIZAR ESTRUCTURA DE TABLA SUBSCRIPTIONS
-- =====================================================

-- 1. Cambiar el tipo de columna access_credentials de TEXT a JSONB
ALTER TABLE public.subscriptions 
ALTER COLUMN access_credentials TYPE JSONB USING 
  CASE 
    WHEN access_credentials IS NULL THEN NULL
    WHEN access_credentials = '' THEN NULL
    ELSE access_credentials::JSONB
  END;
...
WHERE table_name = 'subscriptions' 
  AND column_name = 'access_credentials';

-- 3. Actualizar todas las suscripciones existentes con credenciales de ejemplo
...
WHERE access_credentials IS NULL OR access_credentials = 'null'::jsonb;
...
-- 5. Contar cuántas suscripciones fueron actualizadas
SELECT 
...
-- 6. Verificar que la estructura JSON es correcta
SELECT 
  service_name,
