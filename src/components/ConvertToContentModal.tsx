...
import { Task, ContentItem, ContentType, ContentPlatform } from '../types';

interface ConvertToContentModalProps {
  isOpen: boolean;
  onClose: () => void;
  task: Task | null;
  onConvert: (taskId: string, contentData: {
...
const ConvertToContentModal: React.FC<ConvertToContentModalProps> = ({
  isOpen,
  onClose,
  task,
  onConvert
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="font-medium text-gray-900 mb-3">Información de la Tarea</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{task.description || 'Sin descripción'}</p>
              </div>
              <div>
                <span className="text-gray-600">Archivos:</span>
                <p className="font-medium">
                  {task.completedFiles?.length || 0} archivo(s)
                </p>
              </div>
              <div>
          ...
          {/* Archivos de la tarea - Posicionado entre las primeras secciones */}
          ...
          <div className="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={isConverting || platforms.length === 0 || categories.length === 0}
              className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isConverting ? 'Convirtiendo...' : 'Convertir a Contenido'}
            </button>
          </div>
        </form>
      </div>
...
export default ConvertToContentModal;
